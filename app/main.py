...
from app.alerts.console_alert import ConsoleAlertPlugin
...
async def lifespan(app: FastAPI):
    ...
    # Always register console alert plugin for debugging and testing
    alert_manager.register(ConsoleAlertPlugin())
    logger.info("Console alert plugin registered (always active)")

    # Configure additional alert plugins if environment variables are set
...
async def health_check():
    ...
    return HealthResponse(status="ok" if model_service.is_healthy() else "unhealthy")


if __name__ == "__main__":
    import uvicorn

    uvicorn.run("app.main:app", host=settings.api_host, port=settings.api_port, workers=settings.api_workers, reload=True)
