...
from app.config import get_alert_conditions, get_complex_alert_rules, settings
from app.schemas.models import AnomalyContext, AnomalyResult, ParsedLogRecord
...
class AnomalyDetectionService:
    ...
    def __init__(self, model_service: ModelService, log_parser: LogParserService, alert_manager):
        ...
        logger.info(f"Initialized with alert conditions: {self.alert_conditions}")
    ...
    def detect_and_store_anomalies(self, parsed_logs: List[ParsedLogRecord]) -> List[AnomalyResult]:  # noqa: C901
        """
        ...
        for log in parsed_logs:
            ...
            logger.debug(f"Checking rules for service '{log.service}' with features: {log.features}")
            logger.debug(f"Effective rules for service '{log.service}': {effective_rules}")
            ...
            for key, threshold in effective_rules.items():
                ...
                if log_value is not None and isinstance(log_value, (int, float)) and log_value > threshold:
                    logger.warning(f"Rule violation detected for service '{log.service}': {key} ({log_value}) > {threshold}")
        ...
        if logs_for_model_check:
            log_features = self.log_parser.get_feature_vectors(logs_for_model_check, self.model_service.feature_names)
    ...
    def _check_rate_based_alerts(self, new_anomalies: List[AnomalyResult]) -> set:
        ...
        for anomaly in new_anomalies:
            ...
            logger.debug(
                f"Service '{service}' anomaly count: {len(timestamps)} within window. Rule requires: {rule.get('count')}."
            )
            ...
            if len(timestamps) >= rule["count"]:
                ...
                logger.warning(
                    f"High anomaly rate alert triggered for service '{service}'. "
