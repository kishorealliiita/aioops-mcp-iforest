...
from app.schemas.models import ParsedLogRecord, RawLogRecord
...
class LogParserService:
    ...
    def _parse_regex(self, log: RawLogRecord) -> Optional[Dict]:
        ...
        groups = match.groups()
        data = {
            custom_config["field_mapping"][str(i)]: groups[i]
            for i in range(len(groups))
            if str(i) in custom_config["field_mapping"]
        }
    ...
    def _extract_features(  # noqa: C901
        self, raw_log: str, parsed_data: Dict[str, Any], format_type: LogFormat
        ...
        level_mapping = {"DEBUG": 0, "INFO": 1, "WARN": 2, "WARNING": 2, "ERROR": 3, "CRITICAL": 4, "FATAL": 5}
        features["log_level_numeric"] = int(level_mapping.get(str(parsed_data.get("level", "INFO")).upper(), 1))
