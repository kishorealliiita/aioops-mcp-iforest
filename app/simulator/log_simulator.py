...
def generate_db_log():
    ...
    return {
        "raw_log": (
            f"{timestamp} ERROR query_time={random.randint(10, 6000)}ms "
            f"connection_count={random.randint(1, 200)} error_rate={random.uniform(0, 1):.2f}"
        ),
        "service": "database",
        "source": "postgresql",
...
def generate_app_log():
    ...
    return {
        "raw_log": (
            f"{timestamp} [ERROR] Memory usage: {random.randint(30, 95)}% "
            f"CPU usage: {random.randint(10, 99)}% Thread count: {random.randint(10, 200)}"
        ),
        "service": "application",
        "source": "java_app",
        "format_type": "regex",
        "custom_config": {
            "pattern": (
                ...
                r"Memory usage: (\d+)%\s+CPU usage: (\d+)%\s+Thread count: (\d+)"
            ),
...
def generate_anomalous_web_log():
    ...
    return {
        "raw_log": (
            ...
            f'"response_time": {random.randint(3000, 10000)}, "status_code": 500}}'
        ),
        "service": "web_server",
        "source": "nginx",
        "format_type": "json",
...
def generate_anomalous_db_log():
    ...
    return {
        "raw_log": (
            f"{timestamp} ERROR query_time={random.randint(8000, 15000)}ms "
            f"connection_count={random.randint(500, 1000)} error_rate={random.uniform(0.8, 1.0):.2f}"
        ),
        "service": "database",
        "source": "postgresql",
        "format_type": "key_value",
...
def main():  # noqa: C901
    ...
    while True:
        ...
        logs = [generate_web_log(), generate_db_log(), generate_app_log()]

        # Occasionally add anomalous logs for testing
        if cycle % 5 == 0:  # Every 5th cycle
            print("🚨 Adding anomalous logs for testing...")
            logs.extend([generate_anomalous_web_log(), generate_anomalous_db_log()])
