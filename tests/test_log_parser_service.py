...
def test_parse_regex_log_with_custom_pattern(log_parser):
    ...
    raw_log = RawLogRecord(
        ...
        custom_config={
            "pattern": r"\[(.*?)\] \[(\w+)\] \[(.*?)\] - (.*)",
            "field_mapping": {"0": "timestamp", "1": "level", "2": "service_name", "3": "message"},
        },
    )
    parsed_logs = log_parser.parse_logs([raw_log])
    assert len(parsed_logs) == 1
...
def test_parse_unsupported_format(log_parser):
    ...
    with pytest.raises(ValidationError):
        RawLogRecord(raw_log="<xml><event>some event</event></xml>", service="legacy", source="system", format_type="xml")
...
def test_numeric_extraction_from_string(log_parser):
    ...
    raw_log = RawLogRecord(
        raw_log="response_time=500ms duration=1.5s", service="api", source="gateway", format_type=LogFormat.KEY_VALUE
    )
    parsed_logs = log_parser.parse_logs([raw_log])
    assert len(parsed_logs) == 1
...
def test_batch_parsing_with_mixed_formats_and_errors(log_parser):
    ...
    logs = [
        ...
        RawLogRecord(
            ...
            source="n4",
            format_type=LogFormat.REGEX,
            custom_config={"pattern": r"\[(.*?)\] \[(.*?)\] - Resp: (\d+)", "field_mapping": {"1": "level", "2": "resp_time"}},
        ),
