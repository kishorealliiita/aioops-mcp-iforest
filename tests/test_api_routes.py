...
from app.schemas.models import AnomalyResult, RawLogRecord, LogFormat, FeedbackRecord, ParsedLogRecord
...
def test_stream_multi_source_success(mock_log_parser, mock_ad_service):
    ...
    anomaly = AnomalyResult(raw_log="log1", service="s1", source="src1", timestamp=datetime.now(), 
                          anomaly_score=-0.9, log_level="error", message="", rule_violation=False,
                          features={}, metadata={}, context={})
...
def test_train_model(mock_model_service):
    response = client.post("/api/v1/train", json={"logs": [{"raw_log": "log", "service": "s", "source": "s", "format_type": "json"}]})
...
def test_submit_feedback(mock_feedback_service):
    feedback_data = {
        "feedback": [{
            "log": {"raw_log": "log", "service": "s", "source": "s", "format_type": "json"},
            "is_anomaly": 1
        }]
    }
