...
from app.schemas.models import ParsedLogRecord, RawLogRecord, AnomalyResult
...
def mock_log_parser():
    ...
    parser.get_feature_vectors.return_value = np.array([
        [10, 20],
        [100, 200]
    ])
    return parser
...
def sample_parsed_logs() -> List[ParsedLogRecord]:
    ...
    return [
        ParsedLogRecord(raw_log="log1", service="s1", source="src1", timestamp=datetime.now(), features={'feature1': 10, 'feature2': 20}),
        ParsedLogRecord(raw_log="log2", service="s2", source="src2", timestamp=datetime.now(), features={'feature1': 100, 'feature2': 200})
    ]
...
def test_detect_service_specific_rule_anomaly(
    ...
    violating_log = ParsedLogRecord(
        raw_log="web_server_log", service="web_server", source="nginx",
        timestamp=datetime.now(), features={"response_time": 600}
    )
...
def test_detect_default_rule_anomaly(
    mock_model_service,
    mock_log_parser,
    mock_alert_manager,
    mock_get_alert_conditions
    ...
    violating_log = ParsedLogRecord(
        raw_log="db_log", service="database", source="postgres",
        timestamp=datetime.now(), features={"cpu_usage": 95, "response_time": 200}
    )
...
def test_no_rule_violation(
    ...
    normal_log = ParsedLogRecord(
        raw_log="normal_log", service="web_server", source="nginx",
        timestamp=datetime.now(), features={"response_time": 450, "cpu_usage": 80}
    )
...
def test_rate_based_alerts_are_checked(
    mock_model_service,
    mock_log_parser,
    mock_alert_manager,
    mock_get_alert_conditions
    ...
    logs_to_process = [
        ParsedLogRecord(service="web_server", source="nginx", timestamp=datetime.now(timezone.utc), raw_log="1", features={'f1':1}),
    ]
...
def test_anomaly_storage_and_retrieval(anomaly_detection_service: AnomalyDetectionService, sample_parsed_logs: List[ParsedLogRecord]):
...
def test_threshold_logic(anomaly_detection_service: AnomalyDetectionService, sample_parsed_logs: List[ParsedLogRecord]):
    ...
    model_service.predict.return_value = (np.array([-1, -1]), np.array([-0.7, -0.8])) # Only the second one is a "true" anomaly now
